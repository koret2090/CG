...
class App(QtWidgets.QMainWindow, design.Ui_MainWindow):
    def __init__(self):
        ...
        self.up_horizon = [0] * WIDTH 
        self.bot_horizon = [HEIGHT] * WIDTH
    ...
    def edge_processing(self, x, y, xe, ye):
        if (xe != -1):
            # Если кривая не первая
            self.scene.addLine(xe, ye, x, y)
            self.change_horizon(xe, ye, x, y)
        xe = x
        ye = y
        return xe, ye
    ...
    def change_horizon(self, x1, y1, x2, y2):
        # Проверка вертикальности наклона.
        if (x2 - x1 == 0):
            self.up_horizon[x2] = max(self.up_horizon[x2], y2)
            self.bot_horizon[x2] = min(self.bot_horizon[x2], y2)
            return
        ...
        for x in range(x1, x2 + 1):
            y = round(m * (x - x1) + y1)
            self.up_horizon[x] = max(self.up_horizon[x], y)
            self.bot_horizon[x] = min(self.bot_horizon[x], y)
    ...
    def floating_horizon(self, borders_x, x_step, borders_z, z_step, f, angles):
        ...
        self.up_horizon = [0] * WIDTH  # Верхний.
        self.bot_horizon = [HEIGHT] * WIDTH  # Нижний.
        ...
        while z >= z_start - z_step / 2:
            ...
            flag_prev = self.visability(x_prev, y_prev)
            #
            x_left, y_left = self.edge_processing(x_prev, y_prev, x_left, y_left)
            x = x_start
            while x <= x_end + x_step / 2:
                y_curr = f(x, z)
                x_curr, y_curr = angle_transform(x, y_curr, z, angles)
                # Проверка видимости текущей точки. 
                flag_curr = self.visability(x_curr, y_curr)
                ...
                if flag_curr == flag_prev:
                    ...
                    if flag_curr != 0:
                        ...
                        self.change_horizon(x_prev, y_prev, x_curr, y_curr)
                    ...
                    if flag_curr == 0:
                        if flag_prev == 1:
                            ...
                            xi, yi = intersection(x_prev, y_prev, x_curr, y_curr, self.up_horizon)
                            ...
                            xi, yi = intersection(x_prev, y_prev, x_curr, y_curr, self.bot_horizon)
                        # Отображаем сегмент, от предыдущий точки, до пересечения.
                        self.scene.addLine(x_prev, y_prev, xi, yi)
                        self.change_horizon(x_prev, y_prev, xi, yi)
                    else:
                        if flag_curr == 1:
                            if flag_prev == 0:
                                ...
                                self.change_horizon(xi, yi, x_curr, y_curr) 
                            else: # flag_prev == -1
                ...
                flag_prev = flag_curr
                x += x_step
            x_right, y_right = self.edge_processing(x_prev, y_prev, x_right, y_right)
            z -= z_step

    def prepareForDrawing(self):
        self.scene.clear()
        self.up_horizon.clear()
        self.bot_horizon.clear()
