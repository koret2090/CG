...
class App(QtWidgets.QMainWindow, design.Ui_MainWindow):
    ...
    def initUI(self):
        ...
        self.task_conditions.setText("На плоскости заданы треугольник(координатами своих вершин) и множество точек. \
Найти такие окружности, которые проходят хотя бы через три различные точки множеств \
и для которых хотя бы одна их прямых , проходящих через сторону треугольника \
может быть касательной к окружности. \
Среди полученных окружностей найти такую, для которой \
площадь треугольника, образованного центрами треугольника, \
окружности и точки касания, максимальна")
    ...
    def check_triangle_entry(self):
        ...
        if check:
            ...
            self.triangle_centre(x1, y1, x2, y2, x3, y3)
    ...
    def cicrcle_centre(self, x1, y1, x2, y2, x3, y3):
        ...
        if G != 0:
            ...
            self.radius = sqrt((self.x_circle_centre - x2)**2 +\
                 ((self.y_circle_centre - y2)**2))
                            ...
                            if (self.x_tangent_point != None):
                                square = self.get_square(\
                                    self.x_triangle_centre,self.y_triangle_centre,\
                                         self.x_circle_centre, self.y_circle_centre,\
                                            self.x_tangent_point, self.y_tangent_point)
                                if square > self.square:
                                    ...
                                    self.x1_circle = x_points[i]
                                    self.y1_circle = y_points[i]
                                    self.x2_circle = x_points[j]
                                    self.y2_circle = y_points[j]
                                    self.x3_circle = x_points[k]
                                    self.y3_circle = y_points[k]           
    ...
    def get_square(self, x1, y1, x2, y2, x3, y3):
        # Вычисление координат векторов
        xa = (x2 - x1)   
        ya = (y2 - y1)      
        xb = (x3 - x2)
