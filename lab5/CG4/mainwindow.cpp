...
void MainWindow::activate_pixel(double x, double y)
{
    this->scene->addLine(x, y, x, y, *(this->pen));
}
...
void MainWindow::ordered_list_of_active_edges()
{
    ...
    // используя сканирующие строки, проведенные через каждое
    // y + 0.5 определить для каждого ребра многоугольника
    // наивысшуюю сканирующую строку, пересекаемую ребром
    int edges_amount = this->amount;
    for (int i = 0; i < edges_amount; i++)
    {
        //верхняя и нижняя точки ребра
        ...
        {
            // координата наивысшей строки, пересекающей ребро
            double highest_y_row = round(highest_point.y) - 0.5;
            // -0.5 из-за того, что сканирующие строки проходят
            // через центры растровых строк

            // получаем индекс
            int index = highest_y_row - 0.5;

            // сохранить в связном списке ребра с полями
            // x - начальное значение координат х точек пересечения
            // dy - число сканируемых строк, пересекаемых ребром
            // dx - шаг по х при переходе от одной сканирующей строки к другой
            int dy = highest_point.y - lowest_point.y;
            double dx = (lowest_point.x - highest_point.x) / dy;
            double x = highest_point.x + dx * 1 / 2;   /////////////////

            // Занести ребро многоугольника в y-группу,
            // соответствующей этой строке
            Edge_active edge = make_active_edge(x, dx, dy);
            add_active_edge(&(y_groups[index]), edge);
        }
    }

    // Преобразовать подготовленные данные в растровую форму

    // для каждой сканирующей строки
    for (int i = y_groups_amount - 1; i >= 0; i--)
    {
        // если есть новые активные рёбра,
        // то добавить их в список активных рёбер(САР)
        if (y_groups[i] != NULL)
            add_active_edges(&(y_groups[i]), &active_edges_list);

        // получить список координат x точек пересечения из САР
        list* x_list = NULL;
        fill_x_list(active_edges_list, &x_list);
        // отсортировать элементы списка по возрастранию
        insertion_sort(&x_list);

        list* start = x_list;
        //выделить пары точек пересечений из списка
        while (x_list && x_list->next)
        {
            ...
            double x2 = x_list->x;
            x_list = x_list->next;
            // активировать на текущей строке пикселы для целых
            // значений х таких, что x1 <= x + 0.5 <= x2
        ...
        x_list = start;
        free_x_list(&x_list);
        // Откорректировать САР для перехода на следуюущую строку

        // ребро удаляется из САР при переходе на следующую строку
        //   если следующая строка не пересекает это ребро (dy - 1 == 0)
        //   если следуюущая строка пересекает это ребро в вершине,
        //     которая не является экстремумом
        del_unactive_edges(&(y_groups[i + 1]), &active_edges_list);

        // для всех остальных ребёр САР, не подвергшихся удалению
        // уменьшить dy на 1
        // x = x + dx
...
{
    this->y_groups = (node_t**)malloc(sizeof(node_t*) * HEIGHT);
